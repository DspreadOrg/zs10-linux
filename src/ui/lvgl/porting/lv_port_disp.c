...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    if(disp_flush_enabled) {
        ...
        uint32_t w = area->x2 + 1 - area->x1;
        uint32_t h = area->y2 + 1 - area->y1;
        // POINT_T tStartPoint;
        // tStartPoint.X = area->x1;
        // tStartPoint.Y = area->y1;

        // Disp_vShowImage((unsigned char*)color_p, w * h / 8 * 2, tStartPoint, w, h);
        YMI_DispBlockUpdate(area->x1, area->y1, w, h, (unsigned char*)color_p);
    }
